...
export const INTERNAL_CITATION_RULE = [
  ...
  "- The [short label] is a few DESCRIPTIVE words naming the fact, like a normal inline citation (e.g. [highest richness in rearing water]). NEVER paste the whole passage into the brackets, and NEVER use the token id itself as the label — do not write [P9]{P9}. The label describes the fact; the token points at the evidence.",
  "- WRITE THE LABEL IN THE SAME LANGUAGE AS THE ANSWER. An English answer takes English labels; a Spanish answer takes Spanish labels. Do not copy the language of the examples in this rule — match the answer you are writing.",
