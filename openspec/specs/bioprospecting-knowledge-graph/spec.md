...
The citation graph builder (`src/services/researchBrain/citationGraph.ts`,
backing `GET /api/research-brain/citations/:sourceId`) MUST select its
candidate neighbor set by the UNION (logical OR) of its relatedness
signals: shared canonical compound, shared canonical species, and equal
DOI. DOI equality MUST NOT be applied as an AND-filter that narrows the
candidate query.
...
**Bounding.** The candidate set MUST remain bounded by the existing
`candidateLimit = min(500, limit * 10)` cap so widening the candidate set
does not remove the query's upper bound. Each OR-branch fact scan MUST
itself be bounded by `CITATION_FACT_SCAN_CAP`. A focus source with NO
canonical keys and NO DOI MUST yield an EMPTY candidate set — it MUST NOT
fall through to an unfiltered scan of `research_sources`.

**Overlap semantics.** A candidate's `sharedCompounds` / `sharedSpecies`
MUST be the INTERSECTION of the candidate's canonical keys with the FOCUS
source's canonical keys. A candidate MUST NOT be credited with compounds
or species that the focus source does not itself carry.

`buildCitationGraph` MUST keep its public signature and return shape: it
is composed by `GET /graph/neighborhood` (`related_source` edges) and by
the `/citations/:sourceId` endpoint.

**Defects fixed by this requirement (all three were live in production):**

1. **DOI AND-filter.** `.ilike("doi", sourceDoi)` was applied to the
...
2. **Arbitrary 500-row scan.** When the focus had no DOI, candidates were
   the arbitrary first 500 rows of `research_sources` with no semantic
   filter at all — correct only by accident on a small corpus, and an
   unbounded-scan landmine as the corpus grows.
3. **Un-intersected `sharedCompounds`.** The shared-compound set was never
   intersected with the focus source's own compounds, so weights could
   credit overlap that did not exist.
...
#### Scenario: A focus with no canonical keys and no DOI scans nothing

- GIVEN a focus source with no `compound_canonical_id`, no
  `species_taxon_id`, and no DOI
- WHEN the citation graph is built
- THEN the candidate set is empty
- AND no unfiltered scan of `research_sources` is issued
- AND the result is an empty edge list, not an arbitrary set of sources

#### Scenario: Shared keys are intersected with the focus

- GIVEN a candidate source carrying canonical compounds {A, B} and a focus
  source carrying {B, C}
- WHEN the citation graph is built
- THEN the candidate's `sharedCompounds` is exactly {B}
- AND its `weight` credits one shared compound, not two

#### Scenario: The candidate set stays bounded

- GIVEN a corpus large enough that the OR-union would exceed
  `candidateLimit = min(500, limit * 10)`
- WHEN the citation graph is built
- THEN the candidate set is capped at `candidateLimit`
- AND each branch's fact scan is capped at `CITATION_FACT_SCAN_CAP`
