...
`figure-image-extraction` change. The pre-delta contract above
is preserved unchanged. The delta is additive: the lightbox
renders an `<img>` header for figures with extracted images,
exposes "Open image" / "Download" buttons, splits the bbox
border color (green with image / purple bbox-only), and adds a
"Figure {N} (page {P})" identifier to the header. The
evidence endpoint emits `imageUrl` on each figure when an
image was extracted.
